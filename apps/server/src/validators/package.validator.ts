...
const categoryItem = Joi.object({
  category: Joi.string().valid("standard", "deluxe", "super deluxe").required(),
...
export const createPackageSchema = Joi.object({
  ...
  destination: Joi.array().items(Joi.string().min(2)).min(1).required(),
...
export const updatePackageSchema = Joi.object({
  ...
  destination: Joi.array().items(Joi.string().min(2)),
